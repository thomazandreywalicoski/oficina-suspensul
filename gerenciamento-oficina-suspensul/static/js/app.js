...
(function() {
    ...
    function selecionarFornecedor(id, nome) {
        ...
        if (isProposta) {
            ...
            atualizarObrigatoriedadeMarcaPeca('proposta');
            ...
            atualizarObrigatoriedadeMarcaPeca('os');
            window.calcularVendaPecaOS();
        }
        window.closeModal('modal-seletor-fornecedor');
    }

    function atualizarObrigatoriedadeMarcaPeca(prefix) {
        const fornecedorId = document.getElementById(`peca-${prefix}-fornecedor-id`)?.value;
        const obrigatorio = document.getElementById(`peca-${prefix}-marca-obrigatorio`);
        const marcaInput = document.getElementById(`peca-${prefix}-marca`);
        const isCliente = fornecedorId === 'cliente';
        if (obrigatorio) obrigatorio.style.display = isCliente ? 'none' : 'inline';
        if (marcaInput) {
            marcaInput.disabled = isCliente;
            if (isCliente) marcaInput.value = '';
        }
    }
    window.atualizarObrigatoriedadeMarcaPeca = atualizarObrigatoriedadeMarcaPeca;
    ...
    window.editarPecaOrcamentoProposta = function(idx) {
        ...
        document.getElementById('peca-proposta-marca').value = p.marca || '';
    ...
    window.adicionarPecaOrcamentoProposta = function() {
        ...
        if (!clienteTrouxe && !marca) { showToast('Informe a marca da peça/produto', true); return; }
        ...
        const peca = {
            nome, qtd, marca: clienteTrouxe ? '' : marca, custo: clienteTrouxe ? 0 : custo, lucro: clienteTrouxe ? 0 : lucro, desconto: clienteTrouxe ? 0 : desconto,
        ...
        try {
            ...
            body.innerHTML = `
                <div style="display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:12px;border-bottom:1px solid var(--border-color);">
                    <div>
                        <div style="font-size:18px;font-weight:700;color:var(--text-main);">Comprovante Nº ${String(os.numero).padStart(6, '0')}</div>
                        <div style="font-size:13px;color:var(--text-muted);margin-top:4px;font-weight:400;">Data Pagamento: <span style="color:var(--text-main);font-weight:400;">${os.data_pagamento ? fmtDataBR(os.data_pagamento) : 'Não informada'}</span></div>
                    </div>
                    <span class="tag-entrada" style="background:#2ecc71;color:#fff;padding:4px 12px;border-radius:12px;font-size:11px;font-weight:700;">PAGO</span>
                </div>

                <div class="fin-detalhe-dados" style="display:grid;grid-template-columns:1fr 1fr;gap:20px;background:var(--bg-card);padding:16px;border-radius:8px;border:1px solid var(--border-color);">
                    <div>
                        <div style="font-size:14px;font-weight:700;color:var(--text-main);margin-bottom:8px;padding-bottom:4px;border-bottom:1px solid var(--border-color);">Dados do Cliente</div>
                        <div style="display:flex;flex-direction:column;gap:4px;font-size:13px;color:var(--text-main);">
                            <div><strong style="color:var(--text-muted);font-weight:600;">Nome:</strong> ${escapeHtml(os.nome_completo || 'Não informado')}</div>
                            <div><strong style="color:var(--text-muted);font-weight:600;">CPF:</strong> ${escapeHtml(os.cpf || 'Não informado')}</div>
                            <div><strong style="color:var(--text-muted);font-weight:600;">Whatsapp:</strong> ${escapeHtml(os.whatsapp || 'Não informado')}</div>
                        </div>
                    </div>
                    <div>
                        <div style="font-size:14px;font-weight:700;color:var(--text-main);margin-bottom:8px;padding-bottom:4px;border-bottom:1px solid var(--border-color);">Dados do Veículo</div>
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px 12px;font-size:13px;color:var(--text-main);">
                            <div><strong style="color:var(--text-muted);font-weight:600;">Placa:</strong> ${escapeHtml(os.placa || 'Não informado')}</div>
                            <div><strong style="color:var(--text-muted);font-weight:600;">Ano:</strong> ${escapeHtml(os.ano || 'Não informado')}</div>
                            <div><strong style="color:var(--text-muted);font-weight:600;">Marca:</strong> ${escapeHtml(os.marca || 'Não informado')}</div>
                            <div><strong style="color:var(--text-muted);font-weight:600;">KM:</strong> ${escapeHtml(os.km || 'Não informado')}</div>
                            <div><strong style="color:var(--text-muted);font-weight:600;">Modelo:</strong> ${escapeHtml(os.modelo || 'Não informado')}</div>
                            <div><strong style="color:var(--text-muted);font-weight:600;">Motor:</strong> ${escapeHtml(os.motorizacao || 'Não informado')}</div>
                        </div>
                    </div>
                </div>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:4px;">
                    ...
                    <div style="background:#252525;padding:14px 16px;border-radius:8px;border:none;">
                    ...
                    <div style="background:#2ecc71;padding:14px 16px;border-radius:8px;border:none;color:#ffffff;">
                        <div style="font-size:12px;color:rgba(255,255,255,0.9);font-weight:400;text-align:left;">Lucro Líquido</div>
                        <div style="font-size:16px;font-weight:800;color:#ffffff;text-align:center;width:100%;margin-top:4px;">${fmtBRL(lucroLiquido)}</div>
                    </div>
                </div>
            `;
    ...
    function _renderDividasPessoaCards(dividas) {
        ...
        if (!listEl) return;
        if (!dividas.length) {
            listEl.innerHTML = '<div style="text-align:center;color:var(--text-muted);padding:20px;width:100%;">Nenhuma dívida encontrada.</div>';
        } else {
            listEl.innerHTML = dividas.map(d => {
                ...
                return `<div style="display:grid;grid-template-columns:200px 110px 110px 110px 110px 90px 130px;gap:10px;align-items:center;padding:12px 16px;background:rgba(128,128,128,0.08);border-radius:8px;font-size:13px;min-width:860px;width:100%;box-sizing:border-box;">
                    ...
                    <div style="color:${statusColor};font-weight:600;text-align:center;display:flex;align-items:center;justify-content:center;">${fmtBRL(valorRestante)}</div>
                    <div style="text-align:center;display:flex;align-items:center;justify-content:center;"><span style="display:inline-block;padding:4px 10px;border-radius:12px;font-size:11px;font-weight:700;background:${statusBg};color:${statusColor};">${isPaga ? 'Paga' : 'Pendente'}</span></div>
                    <div style="display:flex;gap:6px;align-items:center;justify-content:center;">
                        <button class="btn-icon btn-action-green no-hover" title="Pagar" ${isPaga ? 'disabled' : `onclick="abrirPagarDivida(${d.id})"`} style="width:36px;height:36px;display:inline-flex;align-items:center;justify-content:center;border-radius:var(--border-radius);padding:0;${isPaga ? 'opacity:0.35;cursor:not-allowed;' : 'cursor:pointer;'}"><i data-lucide="banknote" style="width:18px;height:18px;color:#fff;"></i></button>
